...
+ (void)startEngine {
  ...
  context[@"__weak"] = ^id(JSValue *jsval) {
    ...
    return [[JSContext currentContext][@"_formatOCToJS"]
        callWithArguments:@[ formatOCToJS([JPBoxing boxWeakObj:obj]) ]];
  };
  ...
  context[@"releaseTmpObj"] = ^void(JSValue *jsVal) {
    if ([[jsVal toObject] isKindOfClass:[NSDictionary class]]) {
      ...
      @synchronized(_TMPMemoryPool) {
        [_TMPMemoryPool
            removeObjectForKey:[NSNumber numberWithInteger:[obj hash]]];
      }
    }
  };
  ...
  NSString *path =
      [[NSBundle bundleForClass:[self class]] pathForResource:@"JSPatch"
                                                       ofType:@"js"];
  ...
  if ([_context respondsToSelector:@selector(evaluateScript:withSourceURL:)]) {
    [_context evaluateScript:jsCore
               withSourceURL:[NSURL URLWithString:@"JSPatch.js"]];
  } else {
    [_context evaluateScript:jsCore];
  }
}
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (NSString *jsMethodName in methodDict.allKeys) {
      ...
      NSString *tmpJSMethodName =
          [jsMethodName stringByReplacingOccurrencesOfString:@"__"
                                                  withString:@"-"];
      NSString *selectorName =
          [tmpJSMethodName stringByReplacingOccurrencesOfString:@"_"
                                                     withString:@":"];
      selectorName = [selectorName stringByReplacingOccurrencesOfString:@"-"
                                                             withString:@"_"];

      if (!countArgRegex) {
        countArgRegex = [NSRegularExpression
            regularExpressionWithPattern:@":"
                                 options:NSRegularExpressionCaseInsensitive
                                   error:nil];
      }
      NSUInteger numberOfMatches = [countArgRegex
          numberOfMatchesInString:selectorName
                          options:0
...
#pragma clang diagnostic pop
// OC调用类/对象相应的方法是通过 NSInvocation
// 实现，要能顺利调用到方法并取得返回值，要做两件事：
// 1.取得要调用的OC方法各参数类型，把JS传来的对象转为要求的类型进行调用。
// 2.根据返回值类型取出返回值，包装为对象传回给JS。

/*
 具体实现，以替换 UIViewController 的 -viewWillAppear: 方法为例：
 把UIViewController的 -viewWillAppear: 方法通过 class_replaceMethod()
 接口指向一个不存在的IMP: class_getMethodImplementation(cls,
 @selector(__JPNONImplementSelector))，这样调用这个方法时就会走到
 -forwardInvocation:。
 为 UIViewController 添加 -ORIGviewWillAppear: 和 -_JPviewWillAppear:
 两个方法，前者指向原来的IMP实现，后者是新的实现，稍后会在这个实现里回调JS函数。
 改写 UIViewController 的 -forwardInvocation: 方法为自定义实现。一旦OC里调用
 UIViewController 的 -viewWillAppear: 方法，经过上面的处理会把这个调用转发到
 -forwardInvocation: ，这时已经组装好了一个
 NSInvocation，包含了这个调用的参数。在这里把参数从 NSInvocation
...
static void overrideMethod(Class cls, NSString *selectorName, JSValue *function,
  ...
  if (typeDescription[0] == '{') {
    ...
    if ([methodSignature.debugDescription
            rangeOfString:@"is special struct return? YES"]
            .location != NSNotFound) {
...
static id callSelector(NSString *className, NSString *selectorName,
  ...
  if (isSuper) {
    ...
    NSString *JPSelectorName =
        [NSString stringWithFormat:@"_JP%@", selectorName];
...
#define JP_CALL_ARG_STRUCT(_type, _methodName)                                 \
  ...
  if ([_markArray count] > 0) {
    for (JPBoxing *box in _markArray) {
      ...
      if (obj) {
        @synchronized(_TMPMemoryPool) {
          [_TMPMemoryPool setObject:obj
                             forKey:[NSNumber numberWithInteger:[obj hash]]];
        }
      }
    }
  }
...
#define BLK_ADD_OBJ(_paramName)                                                \
  [list addObject:formatOCToJS((__bridge id)_paramName)];
#define BLK_ADD_INT(_paramName)                                                \
  [list addObject:formatOCToJS(                                                \
                      [NSNumber numberWithLongLong:(long long)_paramName])];
